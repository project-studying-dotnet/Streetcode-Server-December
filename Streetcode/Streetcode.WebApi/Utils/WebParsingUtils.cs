...
namespace Streetcode.WebApi.Utils
{
	public class WebParsingUtils
	{
		...
		public async Task ParseZipFileFromWebAsync()
		{
			...
			var zipPath = $"/src/Streetcode.DAL/InitialData/houses.zip";
		...
		*/
			...
			var streetTypes = new Dictionary<string, string>
		{
			...
			foreach (var streetType in streetTypes)
			{
				if (streetname.Contains(streetType.Key, StringComparison.Ordinal))
				{
					return ConvertToCorrectStreetname(streetname, streetType.Value);
				}
			}
		...
		private static (string, string) ConvertToCorrectStreetname(string streetname, string correctStreetname)
		{
			var substring = streetname.Substring(streetname.IndexOf(" ") + 1);

			return (substring, correctStreetname);
		}
